...
export const DataCenter: React.FC = () => {
  ...
  /**
   * Records a backup entry into both Firestore and LocalStorage
   */
  ...
  const handleExportJson = async () => {
    try {
      ...
      await logBackupAction('Full JSON Backup', fileName, blob.size, {
        clients: clients.length,
        editors: editors.length,
        projects: projects.length,
        payments: clientPayments.length + editorPayments.length,
  ...
  const handleConfirmRestore = async () => {
    ...
    try {
      ...
      // Log safety backup to history
      await logBackupAction('Pre-Restore Safety Backup', safetyFileName, safetyBlob.size, {
        clients: clients.length,
        editors: editors.length,
        projects: projects.length,
        payments: clientPayments.length + editorPayments.length,
  ...
  const handleExportPdf = async () => {
    try {
      ...
      const [firestoreRevisions, firestoreSharedLinks, firestoreRatings] = await Promise.all([
        firestoreService.fetchAllDocumentsInCollection('revisions'),
        firestoreService.fetchAllDocumentsInCollection('sharedLinks'),
        firestoreService.fetchAllDocumentsInCollection('ratings'),
      ]);
      ...
      generateDatabaseSnapshotPdf({
        ...
        revisions: firestoreRevisions,
        sharedLinks: derivedSharedLinks,
        ratings: firestoreRatings.length > 0 ? firestoreRatings : ratings || [],
      });

      const pdfFileName = `Vidzyra-Database-Snapshot-${getBackupTimestamp()}.pdf`;

      // Log PDF export in history
      await logBackupAction('PDF Snapshot', pdfFileName, undefined, {
        clients: clients.length,
        editors: editors.length,
        projects: projects.length,
        payments: clientPayments.length + editorPayments.length,
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Audit trail of all generated JSON backups, PDF snapshots, and automated safety backups.
              </p>
            </div>
          </div>
        ...
        {backupHistory.length === 0 ? (
          <div className="text-center py-10 px-4 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-slate-500">
            ...
            <p className="text-xs text-slate-400 mt-1">
              When you export JSON, download a PDF snapshot, or restore data, records will appear here with timestamps, file size, and entity counts.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {backupHistory.map((item) => {
                  ...
                  const isFull = item.type === 'Full JSON Backup';
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-50/60 transition">
                      ...
                      <td className="py-3 px-3 font-mono text-[11px] text-slate-800 max-w-xs truncate">
                        {item.fileName}
                      </td>
                      <td className="py-3 px-3 text-right font-medium text-slate-600 whitespace-nowrap">
                        {item.fileSizeFormatted || (item.fileSizeBytes ? formatBytes(item.fileSizeBytes) : '—')}
                      </td>
